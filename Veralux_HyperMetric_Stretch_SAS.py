...
class VeraLuxInterface:
    """
    Main GUI Controller (v1.5.2)
    Adapted for SASPro:
    - Uses 'ctx' for logging and undo steps.
    - Removes Siril-specific calls not available in SASPro.
    """
    def __init__(self, ctx, app):
        ...
        # Verify Context
        if not hasattr(self.ctx, 'log'):
            print("Error: Context object missing 'log' method.")
        ...
        self.chk_ontop.setToolTip("Keep this window above SASPro")
        ...
        b_reload.setToolTip("Reload linear image from SASPro memory.")
    ...
    def print_help_to_console(self):
        guide_lines = [
            ...
            "    • RGB input must be Color Calibrated (SPCC) within Siril/SASPro.",
            "",
    ...
    def run_process(self):
        ...
        # We process on a copy, although we don't modify in place usually.
        # But for large images, copy is safe.
        img_copy = self.linear_cache # processed in thread as copy? 
        # VeraLuxCore.normalize_input returns copy or view? Returns float32 copy if type differs.
        
        self.worker = ProcessingThread(
            img_copy, D, b, conv, ws, mode, t_bg, grip, shadow, linear_expansion, adaptive
        )
    ...
    def finish_process(self, result_img):
        ...
        if result_img is not None:
            ...
            # Ensure 3D shape (SASPro preference?)
            # As seen in Veralux_Nox fix:
            if output.ndim == 2:
                 output = np.expand_dims(output, axis=2)
            
            # Set image via SASPro context (handles undo automatically)
            # Use 'open_new_document' if we want new view or 'set_image' for existing?
            # V1.3 used set_image? Let's check user pref. 
            # Usually we update current image OR create new.
            # Veralux_StarComposer used open_new_document.
            # Let's use open_new_document to be safe and separate result.
            
            name = "VeraLux_Result"
            try:
                self.ctx.open_new_document(output, name=name)
                self.ctx.log(f"VeraLux v{VERSION}: {mode} mode applied [{ws}] - New document created.")
            except Exception as e:
                self.ctx.log(f"Save Warning: {e}. Trying simple dictionary args.")
                # Fallback based on learnings from previous task
                try:
                    self.ctx.open_new_document(output, {"name": name})
                except Exception as e2:
                    self.ctx.log(f"Save Failed: {e2}")
